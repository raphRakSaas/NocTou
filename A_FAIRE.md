...
Contexte : NocTou est un **projet personnel**, développé par toi seul, **gratuit, sans publicité, sans monétisation**, aucune structure pro derrière (pas de lien avec "Plum").
...
- [ ] **Bundle identifier `com.plum.noctou`** (`app.json`) : hérité du nom "Plum", qui n'a plus de rapport avec le projet. Je ne l'ai pas touché volontairement car le changer casse tout si l'app est déjà publiée sous cet identifiant (nouvelle fiche store, perte des avis/installs). **À vérifier avant de trancher** : l'app a-t-elle déjà été soumise/publiée avec cet identifiant ?
